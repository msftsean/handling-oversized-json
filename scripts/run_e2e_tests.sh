...
if grep -q "class JsonPreprocessor" OversizedJsonHandler.cs; then
...
if grep -q "public.*FilterRecords" OversizedJsonHandler.cs; then
...
if grep -q "public.*CalculateReduction" OversizedJsonHandler.cs; then
...
if grep -q "var relevantFields = new" Program.cs; then
...
if grep -q "class SemanticChunker" OversizedJsonHandler.cs; then
...
grep -q "ChunkBySeverity\|public.*Chunk" OversizedJsonHandler.cs && chunking_strategies=$((chunking_strategies + 1))
...
if grep -q "IncludesPreviousContext\|ContextTokensReserved" OversizedJsonHandler.cs; then
...
if grep -q "class TokenBudgetManager" OversizedJsonHandler.cs; then
...
if grep -q "ValidateTokenBudget" OversizedJsonHandler.cs; then
...
if grep -q "interface ITokenCounter" OversizedJsonHandler.cs; then
...
if grep -q "class ApproximateTokenCounter" OversizedJsonHandler.cs; then
...
if grep -q "class OversizedJsonOrchestrator" OversizedJsonOrchestrator.cs; then
...
if grep -q "ProcessLargeApiResponseAsync" OversizedJsonOrchestrator.cs; then
...
if grep -q "useContextVaryingPattern\|previousChunkContext" OversizedJsonOrchestrator.cs; then
...
if grep -q "class AnalysisResult" OversizedJsonOrchestrator.cs; then
...
if grep -q "RunSupervisorDashboardExample" Program.cs; then
...
if grep -q "RunDispatcherContextExample" Program.cs; then
...
if grep -q "RunComplianceAnalysisExample" Program.cs; then
...
if grep -q "GenerateSampleIncidents" Program.cs; then
...
doc_comments=$(grep -c "///" OversizedJsonHandler.cs OversizedJsonOrchestrator.cs Program.cs 2>/dev/null || echo 0)
...
if grep -q "try\|catch\|Exception" OversizedJsonOrchestrator.cs Program.cs; then
...
if grep -q "async Task\|await" OversizedJsonOrchestrator.cs Program.cs; then
...
if grep -q "DefaultAzureCredential\|OpenAIClient" OversizedJsonOrchestrator.cs; then
